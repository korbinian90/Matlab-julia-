...
%   MJSE_SHADOW_LIBS - Set to '1' to enable patchelf library shadowing on Linux
    ...
    % Step 2: Library isolation (using LD_LIBRARY_PATH, not renaming)
    ...
    if ~isempty(shadow_libs) && strcmp(shadow_libs, '1') && isunix && ~ismac
        ...
        fprintf('  Note: Julia will use its own libraries via LD_LIBRARY_PATH when launched\n');
        fprintf('  This prevents MATLAB library conflicts without breaking Julia dependencies\n');
    else
